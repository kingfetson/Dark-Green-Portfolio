...
async function handleUpload(e, type) {
  ...
  try {
    ...
    autoFill(text, type);
    showStatus(status, 'Parsed! Review & generate.', 'success');
...
function addWorkExperience() {
  ...
  c.insertAdjacentHTML('beforeend', `
    <div style="background:var(--bg-input);padding:15px;border-radius:var(--radius);margin-bottom:15px;border-left:3px solid var(--green);">
      <input type="text" class="job-title" placeholder="Job Title" style="width:100%;margin-bottom:8px;padding:10px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:8px;color:white;">
      <input type="text" class="company" placeholder="Company" style="width:100%;margin-bottom:8px;padding:10px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:8px;color:white;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px;">
        <input type="text" class="date-start" placeholder="Start" style="padding:10px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:8px;color:white;">
        <input type="text" class="date-end" placeholder="End" style="padding:10px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:8px;color:white;">
      </div>
      <textarea class="job-desc" rows="2" placeholder="Responsibilities..." style="width:100%;padding:10px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:8px;color:white;"></textarea>
    </div>`);
}
...
function addEducation() {
  ...
  c.insertAdjacentHTML('beforeend', `
    <div style="background:var(--bg-input);padding:15px;border-radius:var(--radius);margin-bottom:15px;border-left:3px solid var(--green);">
      <input type="text" class="degree" placeholder="Degree" style="width:100%;margin-bottom:8px;padding:10px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:8px;color:white;">
      <input type="text" class="school" placeholder="School" style="width:100%;margin-bottom:8px;padding:10px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:8px;color:white;">
      <input type="text" class="edu-date" placeholder="Year" style="width:100%;padding:10px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:8px;color:white;">
    </div>`);
}

function addCVExperience() { addWorkExperience(); }
...
function collectResumeData() {
  ...
  document.querySelectorAll('#educationContainer > div').forEach(d => {
    edu.push({ degree: d.querySelector('.degree').value, school: d.querySelector('.school').value, date: d.querySelector('.edu-date').value });
  });
  return {
...
function loadHistory(type) {
  ...
  if (!hist.length) {
    container.innerHTML = '<div class="empty-state"><i class="fas fa-folder-open"></i><p>No history yet.</p></div>';
    return;
  }
...
function previewFromHistory(type, id) {
  ...
  if (!item) return;
  if (type === 'resume') { currentResume = item; renderResumePreview(item); }
  else { currentCV = item; renderCVPreview(item); }
  document.getElementById(`${type}PreviewSection`).style.display = 'block';
